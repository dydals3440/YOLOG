...
❌ **"최적화가 비명을 지를 때"**까지 기다려라
✅ "최적화가 비명을 지를 때"까지 기다려라

❌ 매튜가 이름을 **"익명"**에서 **"매튜"**로 바꿔도
✅ 매튜가 이름을 "익명"에서 "매튜"로 바꿔도

❌ **함수 내부에 갇힌 옛날 `username`**을 참조한다
✅ 함수 내부에 갇힌 옛날 `username`을 참조한다

❌ **항상 다른 `함수`로 인식**되어 리렌더링된다
✅ 항상 다른 함수로 인식되어 리렌더링된다

설명: 백틱 안에 볼드체를 넣거나, 볼드체 안에 백틱을 넣지 않는다.
따옴표가 포함된 문구는 백틱만 사용하거나, 볼드체 없이 따옴표만 사용한다.
