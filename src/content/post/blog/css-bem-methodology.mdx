...
이런 경험이 있다면, `BEM(Block, Element, Modifier)` 방법론이 답이 될 수 있다.

`BEM`은 CSS 클래스 이름을 체계적으로 작성하는 방법론으로, 2009년 러시아의 [Yandex](https://en.bem.info/)에서 개발되어 현재 전 세계적으로 사용되고 있다.

이번 포스트에서는 `BEM`의 세 가지 핵심 개념인 `Block`, `Element`, `Modifier`를 실제 예제와 함께 완벽하게 마스터해보겠다.
...
`Element`는 `Block`의 일부분으로, **Block 안에서만 의미를 가지는 구성 요소**입니다. `Block`과 `Element`는 `__` (더블 언더스코어)로 연결한다.
...
`Modifier`는 `Block`이나 `Element`의 **모양, 상태, 동작을 변경**한다. `--` (더블 대시)로 연결한다.
...
이제 `Block`, `Element`, `Modifier`를 모두 활용해 실제 사용 가능한 카드 컴포넌트를 만들어보겠다.
...
SASS/SCSS를 사용하면 `BEM`을 더 효율적으로 작성할 수 있다:
...
실무에서 AI 도구들과 함께 개발하면서, 예전에 학습했던 `BEM` 방법론을 다시 돌아보게 되었다.

처음 `BEM`을 배울 때는 솔직히 `.card__button--primary` 같은 긴 클래스명이 불필요하게 복잡해 보였다. "그냥 `.btn`으로 써도 되는데..." 라고 생각했었죠.

그런데 최근 `Claude`나 `Cursor` 같은 AI 도구를 활용해 개발하면서 깨달은 점이 있다. **체계적인 규칙이 AI와의 협업을 얼마나 효율적으로 만드는지** 말이죠.
...
특히 이제 막 개발을 시작하는 분들이라면, 처음부터 이런 체계적인 방법론을 익히는 것이 AI 시대에 더 경쟁력 있는 개발자가 되는 지름길이 아닐까 싶다.
