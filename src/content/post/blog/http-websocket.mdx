...
wss.on('connection', (ws) => {
  ...
  // 연결 시 환영 메시지 전송
  ws.send('환영한다! WebSocket 서버에 연결되었다.');
...
WebSocket은 일반 curl로는 테스트하기 어렵다. macOS/Linux에서는 `netcat(nc)` 명령어를 사용해 [TCP](https://www.yolog.co.kr/post/http-tcp) 메시지를 직접 보낼 수 있다:
...
개발자 도구의 Network 탭에서 WebSocket 연결을 아래와 같이 확인할 수 있다.
...
wss.on('connection', (ws) => {
  // 환영 메시지 생성 및 전송
  const welcomeMessage = new Message('채팅방에 입장했다.');
  ...
  // 다른 클라이언트들에게 입장 알림
  broadcast(ws, '새로운 사용자가 입장했다.', true);
...
const initWebSocket = () => {
  ...
  webSocket.addEventListener('close', () => {
    console.log('WebSocket 연결 종료');
    addSystemMessage('서버와의 연결이 끊어졌다.');
  });

  // 에러 처리
  webSocket.addEventListener('error', (error) => {
    console.error('WebSocket 에러:', error);
    addSystemMessage('연결 중 오류가 발생했다.');
  });
};
...
const initSendButton = () => {
  ...
  const sendMessage = () => {
    ...
    if (webSocket && webSocket.readyState === WebSocket.OPEN) {
      ...
      addSystemMessage('연결이 끊어졌다. 새로고침해주세요.');
    }
  };
...
이제 우리가 학습한 모든 HTTP 통신 기법을 비교해보겠다:
