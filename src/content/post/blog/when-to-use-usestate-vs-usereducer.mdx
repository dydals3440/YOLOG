...
export const numberPadReducer = (
  ...
  switch (action.type) {
    case 'INPUT_NUMBER': {
      ...
      if (state.isFirstInput && digit === '0') return state; // 0으로 시작 방지
      if (state.isFirstInput || state.rawValue === '0') {
        newRawValue = digit;
      } else {
        newRawValue = state.rawValue + digit;
      }

      // 3. 유효성 검증
      const numericValue = parseInt(newRawValue);
      if (numericValue > MAX_AMOUNT) {
        return {
          ...state,
          isError: true,
          errorMessage: '최대 10억원까지 가능해요',
        };
      }
      ...
      return {
        ...
        isError: false,
        errorMessage: '',
        history: [...state.history, formatted],
      };
    }

    case 'SET_QUICK_AMOUNT': {
      // 빠른 금액 버튼 - 현재 금액을 완전히 교체
...
const loggingReducer = (state: NumberPadState, action: NumberPadAction) => {
  console.log('Previous State:', state);
  console.log('Action:', action);
  const nextState = numberPadReducer(state, action);
  console.log('Next State:', nextState);
  return nextState;
};

// 타임 트래블 디버깅도 가능
const [state, dispatch, history] = useReducerWithHistory(
  numberPadReducer,
  initialState
);
```
