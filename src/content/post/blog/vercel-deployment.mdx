...
image: '/images/vercel-deployment/thumbnail.webp'
---

![We Go Vercel !!](/images/vercel-deployment/thumbnail.webp)

📅 **이 글은 2026년 1월 28일 기준으로 업데이트되었습니다.** (Vercel CLI 버전 50.5.0 기준)

이후 시점에서 보시는 경우, Vercel 대시보드 UI나 CLI 명령어 옵션이 일부 변경되었을 수 있으나 전체적인 배포 흐름과 핵심 개념은 동일합니다.
...
또 로컬에서 잘 동작하던 기능이 실제 배포 된 환경에서는 예상과 다르게 동작할 수 있습니다.
...
```bash
# Vercel CLI를 전역으로 설치합니다
# -g 플래그는 global 설치를 의미하며, 어느 디렉토리에서나 vercel 명령어를 사용할 수 있습니다
...
```bash
# Vercel 계정으로 로그인합니다
# 브라우저가 열리면서 GitHub, GitLab, Bitbucket 중 하나로 인증할 수 있습니다
vercel login
```
...
# 해당 주소를 통해서 인증을 진행해주세요.
Visit https://vercel.com/oauth/device?user_code=ABCD-EFGH
Press [ENTER] to open the browser
⠹ Waiting for authentication...
```

- 실제로, 해당 브라우저로 이동한 후 사용하실 Vercel 계정으로 로그인을 진행하신 후
- 로그인이 되어있다면, Authorize Device 페이지에서 Allow 버튼을 눌러주시면 됩니다.
- 그 이후에는 Authorization successful 메시지가 표시되면서 로그인이 완료됩니다.
- 터미널에서도, 아래와 같이 나옵니다.

```shell
vercel login                                                     ✔  09:18:09 
Vercel CLI 50.5.0

Visit https://vercel.com/oauth/device?user_code=XDZV-JKDL
...
```shell
# 현재 디렉토리의 프로젝트를 Vercel에 배포합니다
# 처음 실행하면 프로젝트 설정을 위한 대화형 프롬프트가 나타납니다
# 이후 실행부터는 preview 환경으로 자동 배포됩니다
vercel
```
...
**추가 프로젝트 설정 변경**
```shell
? Do you want to change additional project settings? (y/N) # N 입력 후 Enter
```
...
```bash
# 프로덕션 환경으로 배포합니다
# --prod 플래그를 사용하면 실제 사용자가 접근하는 production URL로 배포됩니다
# preview와 달리 production은 한 번에 하나의 배포만 활성화됩니다
vercel --prod
```
...
![Vercel Dashboard 페이지](/images/vercel-deployment/dashboard-vercel.webp)

**Deployments** 탭에 들어가시면 여러분들이 Production에 이제 배포했음을 확인할 수 있습니다.

클릭해서 들어가보시면 **Deployment Details**를 아래와 같이 확인할 수 있습니다.

![Vercel Deployment Details 페이지](/images/vercel-deployment/dashboard-vercel-detail.webp)
...
1. 대시보드로 간 후 여러분들의 프로젝트 명을 클릭해주세요 (ohtani)
   ![첫번째. 프로젝트 명을 클릭해주세요](/images/vercel-deployment/first.webp)

2. 그 후 settings를 클릭해주세요.
   ![두번째. 프로젝트 명을 클릭해주세요](/images/vercel-deployment/second.webp)

3. `Environment Variables` 탭에서 여러분들의 `환경변수 key`와 `value`에 맞게 값을 넣어주셔도 되고 많은 경우 그냥 `.env` 파일자체를 첨부하셔도 좋습니다. 그 이후 save를 해주세요.
   ![세번쨰. 환경 변수 첨부](/images/vercel-deployment/third.webp)
...
대시보드 UI 외에도 CLI로 직접 환경변수를 추가할 수 있습니다. 이 방식은 자동화 스크립트나 팀 온보딩에 유용합니다.

```bash
# production 환경에 VITE_API_URL 환경변수를 추가합니다
# 명령어 실행 후 값을 입력하라는 프롬프트가 나타납니다
vercel env add VITE_API_URL production
```

명령어를 실행하면 값을 입력하라는 프롬프트가 나옵니다. 여기에 `https://ohtani.example.com` 같은 실제 값을 붙여넣으면 됩니다.

```bash
Vercel CLI 50.5.0
WARN! VITE_ variables can be seen by anyone visiting your site
? Your value will be encrypted. Mark as sensitive? no
? What's the value of VITE_API_URL? *********************
✅  Added Environment Variable VITE_API_URL to Project zz [213ms]
...
```bash
# production과 preview 환경 모두에 STRIPE_SECRET_KEY를 추가합니다
# 한 번의 명령어로 여러 환경에 동일한 변수를 설정할 수 있습니다
...
```bash
# 현재 프로젝트의 모든 환경변수 목록을 확인합니다
# 각 변수가 어느 환경(production, preview, development)에 설정되어 있는지 보여줍니다
vercel env ls
```

CLI 방식의 장점은 `.env` 파일을 하나씩 복사-붙여넣기하지 않아도 되고, 스크립트로 자동화할 수 있다는 점입니다.

예를 들어 팀 새 멤버가 프로젝트를 클론했을 때, 로컬 환경변수를 한 번에 받아올 수도 있습니다:

```bash
# Vercel에 설정된 환경변수를 로컬 .env.local 파일로 다운로드합니다
# 팀 멤버가 프로젝트를 클론했을 때 환경변수를 쉽게 동기화할 수 있습니다
# 기존 .env.local 파일이 있으면 덮어쓰므로 주의해야 합니다
vercel env pull .env.local
```

이 명령어는 Vercel에 설정된 환경변수를 로컬 `.env.local` 파일로 다운로드합니다.

더 많은 CLI 명령어는 [Vercel CLI 공식 문서](https://vercel.com/docs/cli)를 참고하세요.
...
![홈페이지는 의도한대로 잘 나오군..!](/images/vercel-deployment/intention-situation.webp)

하지만 `/baseball` 페이지는 실제로 개발환경에서는 잘 보이나 배포된 환경에서는 보이지 않습니다.

![어라라..! 왜.. 이건](/images/vercel-deployment/unintention-situation.webp)
...
## ✅ 해결 방법: vercel.json으로 모든 경로를 index.html로 보내기

감사하게도 `vercel`은 이러한 부분을 쉽게 해결할 수 있도록 해결 방법을 공식문서에 제공해주었습니다.

프로젝트의 루트에 `vercel.json` 파일을 만들어줍니다.

![나의 루트 파일](/images/vercel-deployment/root-file.webp)

그 후 아래와 같이 코드를 작성해줄 것입니다.
...
- [`$schema: "https://openapi.vercel.sh/vercel.json"`](https://vercel.com/docs/projects/project-configuration#schema)
  👉 IDE에서 자동완성을 지원하고 설정 검증을 도와줍니다.

  <br />

- `source: "/(.*)"`
  👉 모든 경로를 잡습니다. `(.*)`는 와일드카드 패턴으로 어떤 경로든 매칭됩니다.
  `/about`, `/baseball` 같은 페이지 경로는 물론, `/blog/post/123` 같은 중첩 경로도 모두 처리합니다.

  <br />

- `destination: "/index.html"`
  👉 매칭된 모든 경로는 `index.html`을 반환합니다.
  브라우저가 `index.html`을 로드하면, React 라우터가 URL을 보고 적절한 컴포넌트를 렌더링합니다.

### 참고: 레거시 routes 방식도 여전히 작동합니다

Vercel 이전 버전에서는 `routes` 설정을 사용했습니다.
...
이 방식도 여전히 동작하지만, 최신 `rewrites` 방식이 더 명확하고 프레임워크 설정과의 일관성이 좋습니다.
특히 `rewrites`는 [Vercel 공식 문서](https://vercel.com/docs/projects/project-configuration#rewrites)에서 권장하는 표준 방식입니다.

기존 프로젝트를 유지보수하는 경우라면 `routes`를 그대로 사용해도 괜찮지만,
새로운 프로젝트라면 `rewrites`를 사용하는 것이 좋습니다.
...
![야호! 🍠](/images/vercel-deployment/check-success.webp)
...
`Domains` 탭에 들어간 후
![도메인 탭 들어가기](/images/vercel-deployment/domain-first.webp)

`Add Domain` 버튼 클릭 이후
여러분들이 가비아나, GoDaddy 등을 통해
URL을 구매한 후 해당 도메인을 입력해줍니다.
![도메인 입력하기](/images/vercel-deployment/domain-second.webp)

그 후 다시 `Add Domain`을 눌러주세요.
![도메인 추가하기](/images/vercel-deployment/domain-third.webp)

별다른 설정없이 일단은 Save를 진행하겠습니다.
![도메인 저장하기](/images/vercel-deployment/domain-fourth.webp)

이렇게 이제 `A 레코드`와, `CNAME 레코드`를 주게 되는데
해당 값을 이제 해당 도메인을 구매한 곳에서 `DNS 레코드 설정`을 해주시면 됩니다.
![저는 가비아에서 구매해서 DNS 설정을 여기서 했습니다!](/images/vercel-deployment/domain-fifth.webp)

### 도메인 소유권 검증이 필요한 경우도 있다

경우에 따라 Vercel이 TXT 레코드를 통한 도메인 소유권 검증을 요구할 수 있습니다.
이는 해당 도메인이 이미 다른 Vercel 계정에 연결되어 있거나, 보안상 추가 검증이 필요할 때 발생합니다.

Vercel 대시보드에서 제공하는 TXT 레코드 값을 DNS 설정에 추가합니다:
...
TXT 레코드는 도메인에 텍스트 정보를 추가하는 DNS 레코드 타입입니다.
주로 다음과 같은 용도로 사용됩니다:

- 도메인 소유권 검증 (Vercel, Google Search Console 등)
- 이메일 인증 (SPF, DKIM, DMARC)
- 사이트 검증 (각종 웹 서비스)

TXT 레코드를 추가한 후 Vercel 대시보드에서 `Verify` 버튼을 누르면 검증이 진행됩니다.
검증이 완료되면 도메인이 정상적으로 연결됩니다.

DNS 전파는 보통 몇 분 이내에 완료되지만, 최대 48시간이 걸릴 수 있다.
급하게 배포해야 한다면 DNS 전파를 기다리는 동안 Vercel이 제공하는 기본 `.vercel.app` 도메인을 먼저 사용할 수도 있다.

### DNS 전파 시간 이해하기

DNS 설정을 변경하면 전 세계 DNS 서버에 변경사항이 전파되는 시간이 필요합니다.
이는 각 DNS 서버의 캐시 정책(TTL, Time To Live)에 따라 다릅니다.

전파 상태를 확인하려면 다음 명령어를 사용할 수 있습니다:

```bash
# A Record 확인: 도메인이 어떤 IP 주소로 연결되어 있는지 확인합니다
dig example.com

# CNAME Record 확인: 서브도메인이 어떤 도메인으로 연결되어 있는지 확인합니다
dig www.example.com
```

또는 온라인 도구인 [DNS Checker](https://dnschecker.org/)에서 전 세계 여러 지역의
DNS 전파 상태를 실시간으로 확인할 수 있습니다.
...
A Record는 도메인을 IP 주소에 직접 연결하는 방식입니다.
...
`@`는 루트 도메인(예: `example.com`)을 의미합니다.
사용자가 `example.com`을 입력하면 DNS가 `76.76.21.21` 서버로 연결합니다.

A Record는 고정된 IP 주소를 사용하므로 직접적이고 빠르지만,
Vercel이 서버 IP를 변경하면 수동으로 업데이트해야 한다는 단점이 있습니다.

#### CNAME Record (Canonical Name Record)

CNAME은 도메인을 다른 도메인에 연결하는 별칭(alias) 방식입니다.
...
`www.example.com`을 `cname.vercel-dns.com`으로 연결합니다.
사용자가 `www.example.com`에 접속하면, DNS가 먼저 `cname.vercel-dns.com`을 찾고,
Vercel이 자동으로 적절한 서버 IP로 라우팅합니다.

CNAME의 장점은 Vercel이 서버 인프라를 변경해도 `cname.vercel-dns.com`이 자동으로
새로운 IP로 업데이트되므로 우리가 DNS 설정을 건드릴 필요가 없다는 점입니다.

#### 왜 두 가지 모두 필요한가?

- **A Record** (`@`): `example.com` (루트 도메인)
- **CNAME Record** (`www`): `www.example.com` (서브도메인)

DNS 표준상 루트 도메인(`@`)에는 CNAME을 사용할 수 없어서 A Record를 사용하고,
서브도메인(`www`, `blog` 등)에는 CNAME을 사용하는 것이 일반적입니다.

이렇게 설정하면 `example.com`과 `www.example.com` 둘 다 Vercel로 연결됩니다.
...
![짜잔 ✨](/images/vercel-deployment/domain-seventh.webp)

이제 실제로 여러분들이 구매하신 도메인으로 접속이 가능합니다!

![배포 성공한 URL을 신중히 보셔야 합니다 ✨](/images/vercel-deployment/deployment-success.webp)

여러분들도 만드신 사이트를 이런 식으로 세상에 띄워보고 싶지 않으신가요?

제가 사이드 프로젝트로 만든 `아이두(Aido)`의 공식 웹사이트인 [aido.kr](https://www.aido.kr/)도 지금까지 설명한 과정 그대로 배포되어 있습니다.

Vercel에 프로젝트를 올리고, 직접 구매한 도메인을 `A 레코드`와 `CNAME 레코드`로 연결한 것이 전부입니다.

아이두는 친구들과 함께 할 일을 관리하며 서로 자극을 주고받을 수 있는 AI 할일 관리 서비스인데, **iPhone, Android, Mac, Tablet 등 다양한 기기에서 호환 가능**하니 한 번 써보시면 좋을 것 같습니다!

혹여나 주변에 같이 쓸 친구가 없다면 초대 코드 `WNTQJEEE`로 들어와서 함께 자극을 나누며 성장하면 좋을 것 같습니다!
...
특히 환경변수를 추가하거나 `vercel.json` 설정을 변경한 후에는 재배포가 필요합니다.
이때 새로운 배포를 만들지 않고 기존 배포를 다시 빌드하려면 `vercel redeploy` 명령어를 사용할 수 있습니다.

```bash
# 특정 배포를 다시 빌드하고 배포합니다
# 코드 변경 없이 환경변수나 vercel.json 설정만 바뀌었을 때 유용합니다
# URL은 Vercel 대시보드의 Deployments 페이지에서 확인할 수 있습니다
vercel redeploy https://your-deployment-url.vercel.app --prod
```

이 명령어는 코드 변경 없이 설정만 바뀌었을 때 유용합니다.
예를 들어 환경변수를 추가했는데 코드는 그대로일 때, 불필요한 Git 커밋 없이 바로 재배포할 수 있습니다.
...
해당 부분은 [GitHub CI/CD 파이프라인으로 Vercel 자동 배포하기](/post/github-cicd-vercel) 글에서 이어서 다뤘습니다.

이 글에서 손으로 입력했던 `vercel`, `vercel --prod`를 GitHub Actions가 대신 실행해주기 때문에, 코드를 푸시하는 것만으로 배포가 끝나게 됩니다.

---

📚 **이어지는 글:**

- [GitHub CI/CD 파이프라인으로 Vercel 자동 배포하기](/post/github-cicd-vercel)

---

이 글을 읽고, 이 방식으로 배포해 운영하고 있는 저희 서비스에 관심이 생기셨다면 아래 링크를 통해 다운을 받아서 활용해보세요!
