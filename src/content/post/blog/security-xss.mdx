...
하지만 때로는 사용자가 입력한 내용을 그대로 보여주되, `HTML`로 해석되지 않게 하고 싶을 때가 있습니다.
...
이제 `<h1>왕 고구마</h1>`가 그대로 텍스트로 출력됩니다! 이렇게 하면 브라우저가 이를 `HTML`로 해석하지 않고 그대로 텍스트로 출력합니다.
...
`XSS`의 핵심은 **신뢰할 수 없는 출처의 스크립트가 신뢰받는 사이트에서 실행되는 것**입니다.
...
`XSS`는 단순한 `alert`창을 띄우는 것 이상의 심각한 공격이 가능합니다:
...
이러한 고급 공격 기법들은 [**세션 하이재킹**](https://www.yolog.co.kr/post/security-session-hijacking) 포스트에서 실습을 통해 자세히 다루고 있습니다.
