...
우리 회사에서 만들고 있는 서비스는 다양한 권한이 복잡하게 얽혀 있는 서비스다.

사실 처음에는 **RBAC(Role-Based Access Control)** 이라는 용어만 알고 있었는데, 서비스가 점점 커지면서 **ABAC(Attribute-Based Access Control)** 이라는 개념이 있다는 걸 알게 됐다.

이번 포스팅에서는 **RBAC과 ABAC의 간단한 차이**를 정리하고, 우리 서비스 코드에서 **ABAC** 권한 관리를 어떻게 처리했는지 기존의 문제점을 개선한 내용에 대해 공유해보려고 한다.
...
- **RBAC (Role-Based Access Control)**
  → **"이 사람의 역할이 무엇인지"** 에 따라 권한을 부여하는 방식
  ex: `이 사람이 ADMIN이면 글 삭제 가능`

<br />

- **ABAC (Attribute-Based Access Control)**
  → **"이 사람과 이 리소스가 어떤 속성을 갖고 있는지"** 에 따라 권한을 부여하는 방식
  ex: `이 사람이 PRINCIPAL이고, 해당 학원의 canDeleteChallenge 속성이 true면 글 삭제 가능`

초기엔 **RBAC**이 심플하고 좋지만, 서비스가 커질수록 **"역할만으로 설명이 안 되는 경우"** 가 점점 늘어난다. 결국 **ABAC** 형태로 확장할 수밖에 없다.
...
처음엔 그냥 **RBAC이면 충분하겠지** 싶었는데, 결국 서비스가 커지면서 **RBAC**을 효율적으로 처리하는 방법을 유튜브나, Medium 글을 찾아보며 **ABAC** 개념에 대해서 알게 되었고, 훨씬 권한 관리를 효율적으로 처리할 수 있는 패턴을 배우게 되었다.

혹시 나처럼 서비스 만들다 **권한 때문에 골치 아픈 사람**이라면 위의 패턴을 한번 활용해보면 좋겠다! 정말 효율적으로 코드를 쉽게 작성할 수 있게 된다! ✨

사실, 조직 구조가 복잡해지고 역할의 수가 기하급수적으로 증가하는 현상을 당근 테크에서 올린 게시글에서는 `Role Explosion` 이라고 표현하고 있다.

그래서 이를 해결하기 위해 **구글의 Zanzibar**에서 활용하는 **ReBAC(Relation-Based Access Control)** 이라는 개념을 소개하고 있으니, 관심있으신 분들은 한번 참고해보면 좋을 것 같다.

[당근 테크 글 링크](https://medium.com/daangn/%EA%B5%AC%EA%B8%80%EC%B2%98%EB%9F%BC-%EB%B3%B5%EC%9E%A1%ED%95%9C-%EA%B6%8C%ED%95%9C-%EC%89%BD%EA%B2%8C-%EA%B4%80%EB%A6%AC%ED%95%98%EA%B8%B0-feat-graphql-9ce80d34d39b/)
