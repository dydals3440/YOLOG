...
category: HTTP
image: '/images/http-content-negotiation/thumbnail.webp'
---
...
![HTTP Content Negotiation](/images/http-content-negotiation/thumbnail.webp)
...
- [`Accept`](https://developer.mozilla.org/ko/docs/Web/HTTP/Headers/Accept) ↔ [`Content-Type`](https://developer.mozilla.org/ko/docs/Web/HTTP/Headers/Content-Type): 미디어 타입 협상
- [`Accept-Encoding`](https://developer.mozilla.org/ko/docs/Web/HTTP/Headers/Accept-Encoding) ↔ [`Content-Encoding`](https://developer.mozilla.org/ko/docs/Web/HTTP/Headers/Content-Encoding): 압축 방식 협상
- [`Accept-Language`](https://developer.mozilla.org/ko/docs/Web/HTTP/Headers/Accept-Language) ↔ (문서/리소스 선택): 언어 협상

> [`Vary: Accept-Encoding`](https://developer.mozilla.org/ko/docs/Web/HTTP/Headers/Vary)은 거의 항상 기본 적용되지만, `Vary: Accept-Language`를 적용하면 `CDN 캐시`가 언어별로 따로 저장되므로 캐시 효율이 떨어질 수 있음을 주의해야 한다.
...
마치 레스토랑에서 같은 재료로 스테이크를 주문할 때, 어떤 손님은 웰던을, 어떤 손님은 미디엄 레어를 원하는 것과 같다. 서버는 손님의 요구를 듣고 가장 적합한 형태로 제공한다.
...
마치 택배를 보낼 때 박스를 압축 포장하는 것과 같다. 받는 사람이 박스를 풀 수 있다면(압축 해제 가능), 더 적은 공간으로 더 빠르게 배송할 수 있다.
...
마치 글로벌 콜센터에서 고객이 "한국어로 상담받고 싶어요"라고 말하면, 한국어를 할 수 있는 상담원으로 연결해주는 것과 같다.
...
클라이언트는 [`User-Agent`](https://developer.mozilla.org/ko/docs/Web/HTTP/Headers/User-Agent)로 자신을 소개한다. 다만 이 값은 신뢰하기 어려워(우회/변조 가능) 현대적인 콘텐츠 협상에는 적합하지 않다. 기능 지원 여부는 UA 스니핑보다 **기능 감지(Feature Detection)** 나 **표준 헤더 기반 협상**을 권장한다.
...
app.use((req, res, next) => {
  ...
  if (isIE) {
    res.status(400).send('<html><body>IE는 지원하지 않다.</body></html>');
    return;
  }
  next();
});
...
- **콘텐츠 협상이란**: 클라이언트와 서버가 헤더를 통해 최적의 콘텐츠 형식을 결정하는 과정
- **주요 협상 헤더**:
  - `Accept` ↔ `Content-Type`: 미디어 타입 (HTML, JSON 등)
  - `Accept-Encoding` ↔ `Content-Encoding`: 압축 방식 (gzip, br 등)
  - `Accept-Language`: 언어 선호도 (ko, en 등)
- **Vary 헤더**: 어떤 요청 헤더에 따라 응답이 달라지는지 명시 (캐시 제어)
- **실무 활용**: Express의 `res.format`, `compression`, `negotiator` 등으로 구현
- **주의사항**: `User-Agent`는 신뢰하기 어려우므로 기능 감지 권장

**콘텐츠 협상은 레스토랑에서 손님의 주문을 듣고 가장 적합한 요리를 제공하는 것과 같다. 클라이언트가 원하는 형식을 요청하면, 서버는 가능한 한 그에 맞춰 응답한다.**
