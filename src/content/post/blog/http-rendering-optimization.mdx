...
브라우저가 웹 페이지를 렌더링하려면 이미지, 글꼴, 스타일시트 같은 <Highlight>추가 자원</Highlight>이 필요합니다. 이 자원들은 대부분 원격 서버에 있어서 <Highlight color="blue">HTTP 요청</Highlight>으로 불러와야 합니다.

이런 추가 요청은 <Highlight color="orange">렌더링 성능</Highlight>에 큰 영향을 줍니다. 웹 페이지가 빠르게 로드되려면 이러한 HTTP 요청을 효율적으로 제어해야 합니다.
