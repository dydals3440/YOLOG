...
웹 문서를 받은 브라우저는 이제부터 이 문서를 사람이 이해할 수 있는 형태로 화면에 그려야 합니다. 여기서부터 주요 렌더링 경로, `Critical Rendering Path(CRP)`라고 합니다. CRP 렌더링 과정이 시작됩니다. 먼저 파싱 단계입니다.
...
큰 파일을 로딩하고, 그 이어서 작은 파일을 로딩하는 간단한 스크립트를 로딩하게 하는 것입니다.
...
이 파일을 서버에서 제공을 해야하니, 같은 이름으로 하나 만들어보겠습니다.
...
두 자바스크립트는 이렇게 HTML에 로딩되면 각자의 파일 이름을 콘솔에 기록할 겁니다. 두 자바스크립트는 이름처럼 응답 시간을 좀 달리해야 합니다. 그래야 이게 무거운 파일인지, 가벼운 파일인지 흉내낼 수 있기 때문입니다. 정적 파일을 처리하는 이 shared의 serve-static.js 파일로 가보겠습니다.
...
이제 루트로 들어오는 public 폴더에 있는 파일들을 읽어서 응답해주는 그런 핸들러입니다. 핸들러 맨 아래쪽 로직을 보면 본문을 실는 `write` 함수 위쪽에다 인터페이스를 하나 열 것입니다. `delayMs`에서 응답 지연 시간을 response 객체의 이 속성으로 받는 것입니다.

`res.delayMs`만큼 지연하고 아래 있는 것처럼 본문을 싣고 응답을 종료하는 로직입니다. 그리고 이 분기문 이 로직을 빠져나오겠습니다. 그래서 이 앞단에서 `delayMs`의 시간을 밀리초 단위로 지정을 하면 그만큼 이 서버는 응답을 지연해서 보낼 것입니다.

그러면 이렇게 인터페이스를 열었으니 server.js로 돌아가보겠습니다.
...
script-big.js가 로딩되고 이어서 small.js가 로딩됐습니다.

브라우저는 http://localhost:3000번으로 요청을 하면 서버는 HTML을 브라우저로 응답을 합니다.

브라우저가 파일의 내용을 읽으면서 한 줄씩 파싱하기 시작합니다. 그러다가 이 스크립트 로딩 태그를 만나게 되면 파싱을 멈춥니다.

왜냐하면 이 `src`에 있는 script-big.js 파일을 가져오기 위해서 네트워크 요청을 보내기 때문입니다. 서버는 한 3초 정도 후에 이 파일을 브라우저까지 전달을 해줍니다. 그러면 이 브라우저는 이 파일을 실행해서 script-big.js라는 콘솔 로그를 기록합니다. 이렇게 해서 브라우저는 스크립트 하나를 처리했습니다.

그리고 나서 파싱을 재개하고 다시 스크립트 로딩 태그를 만납니다. 위에 했던 것처럼 똑같이 파싱을 멈추고 script-small.js를 가져오기 위한 네트워크 요청을 만듭니다.

![script-big.js, script-small.js를 가져오기 위한 네트워크 요청을 만든다](/images/http-rendering-optimization/3.png)

이렇게 스크립트 로딩을 완료하고 브라우저는 멈췄던 파싱을 다시 재개합니다. 그래서 한 줄 한 줄씩 읽고 마침내 `DOM` 객체를 완성합니다. 이 브라우저가 렌더링하기 위해서 문서를 파싱하는데 이 스크립트 태그를 처리하기 위해서 대략 4초 정도의 시간을 소요했습니다.

script-big.js와 script-small.js를 가져오고 그리고 나서 둘 다 실행하는데 대략 4초 정도의 시간이 소요됐습니다.

조금 더 정확하게 시간을 측정해보겠습니다. 브라우저의 이 Performance API를 사용하면 밀리초 단위로 시간을 잴 수 있습니다.

먼저 스크립트를 로딩하기 직전에 `window`에서 제공해주는 Performance API를 써보겠습니다.

아래와 같이 `performance.mark`를 통해 측정 시작을 표기하고
...
아래에 이제 `performance.mark`를 통해 측정 마무리를 표기 후, `performance.measure`를 통해 실제로 측정을 시작합니다.
...
각각 표기해주고, 그 후에 이제 브라우저는 필요한 모든 리소스를 로딩하면 로드 이벤트를 발행합니다. 이 시점에 웹 문서와 모든 자바스크립트가 로딩을 마친 것입니다. 그래서 이 이벤트를 구독해서 결과를 표시하는 코드도 추가하겠습니다.
...
바로 이 script 태그의 `async` 속성인데요, 이 속성을 브라우저가 만나면 I/O 스레드에게 이 스크립트 다운로드를 부탁하고, 메인 스레드로는 계속 파싱을 이어서 합니다. 다운로드를 함과 동시에 파싱도 멈추지 않고 계속 진행할 수 있어서 처리 시간을 단축하는 효과가 있습니다. `async` 속성을 스크립트에 추가해보겠습니다.
...
위의 이미지를 보면은 이 스크립트 파일을 동시에 다운로드함을 확인할 수 있습니다. 29ms에 동시에 다운로드함을 확인할 수 있습니다. 메인 스레드가 `DOM`을 먼저 만들고, 좀 더 가벼운 small.js가 먼저 다운로드가 되었습니다. 그러면 메인 스레드는 이걸 처리합니다. 이 스크립트를 실행해서 console.log를 출력한 것입니다. 그게 약 1초 정도에 찍혔습니다. 이어서 script-big도 다운로드가 완료되면 메인 스레드가 이것도 실행합니다.

![기존 1초 + 3초 = 4초 이지만, async 속성시 3초 소요](/images/http-rendering-optimization/7.png)

그래서 console.log를 찍고 그 시간이 한 3초 정도 걸렸습니다. 그래서 총 시간은 이전 4초에서 3초 정도로 줄었습니다. 네트워크를 봐도 script-big과 script-small이 동시에 다운받음을 확인할 수 있습니다.
...
이런 `async` 스크립트는 `DOM`에 무관한 스크립트 그리고 서로 영향을 주지 않은 스크립트에 적합합니다. 왜냐하면 코드의 명시한 순서가 아니라 다운로드 된 순서대로 실행되기 때문입니다.

그래서 애플리케이션에 무관한 광고 스크립트나, 분석 트래커에다가 `async`로 스크립트를 로딩하면 좋습니다.
...
이런 자바스크립트가 서로 의존적일 경우에는 이런 `async`가 적당하지 않을 수 있습니다. script-big에 아래와 같은 게 있다고 해보겠습니다. `foo`라는 함수를 `window` 객체에 정의하겠습니다. 누군가 이거를 쓰도록 의도한 겁니다.
...
이것처럼 `defer`는 서로 영향 있는 스크립트에 적합합니다. 애플리케이션을 만들 때 webpack 같은 번들러 도구를 많이 사용하는데 필요에 따라 chunk라고 부르는 작은 자바스크립트 파일로 쪼개서 빌드합니다. 여러 개 chunk 파일은 HTML에 모두 로딩이 돼야 하는데 의존성에 따라서 순서대로 로딩이 되야 합니다.
...
<body>
  ...
  <script>
    ...
    document.querySelector('#imageAddButton').addEventListener('click', () => {
      ...
      // 주소를 goguma.jpg로 해서 고구마 이미지를 로딩하자.
      img.src = '/goguma.jpg'
      // 그래서 이 imgElement를 body에 추가하면 브라우저는 goguma.jpg를 가져오기 위한 네트워크 요청을 만듬.
...
`preload`는 미리 다운로드해야 될 어떤 자원이든 다 적용할 수 있습니다. 지금처럼 이미지도 되고, 비디오, 스타일시트, 폰트, 자바스크립트 등에도 이렇게 사용할 수 있습니다.
...
if (filename === 'goguma.html') {
  ...
  res.delayMs = 3_000;
  // prefetch 고려해서 헤더 추가
  // Chrome 같은 브라우저는 없어도 잘 동작하지만, Firefox는 Prefetch하기 위해서 응답 헤더를 주어야 함.
...
```html
<head>
  <link rel='prefetch' href='goguma.html' as='html' />
</head>
```
...
Firefox는 아까 보셨던 것처럼, 이렇게 캐시 헤더가 있어야 동작합니다. Chrome은 이런 것과 무관하게 잘 동작했습니다. 애플리케이션의 브라우저 지원 범위를 고려하면서 활용해야 합니다. [Can I Use에서 prefetch 브라우저 호환성](https://caniuse.com/link-rel-prefetch)을 확인할 수 있습니다.
...
Gatsby.js는 자체 빌드 설정으로 이렇게 프리페치 기능을 사용합니다. webpack도 prefetch를 사용합니다. 모듈을 불러올 때 주석으로 아래와 같이 `webpackPrefetch: true`라고 쓰면

```js
import(/* webpackPrefetch: true */ './path/to/LoginModal.js');
```

코드를 빌드할 때 링크 프리페치를 코드에 넣어주는 기능도 있습니다. Next.js 프레임워크는 자체 제작한 Link 컴포넌트가 있는데 이걸 사용하면 prefetch 기능을 제공한다고 합니다.
...
저 어딘가 끝에 이제 두 번째 고구마 이미지가 있을 것입니다.

네트워크 탭을 보면 고구마 두 개의 이미지 모두 네트워크 요청이 만들어졌습니다.

사용자 입장에서는 1번 고구마 이미지만 있어도 충분합니다. 왜냐하면, 두 번째 고구마 이미지는 그냥 저 아래 구석에 있기 때문에 사용자가 볼 확률이 매우 적기 때문입니다.

사용자가 스크롤하지 않고, 다른 페이지로 이동하면 정말 필요 없는 자원이 됩니다.

그래서 뷰포트 안에 이미지만 다운로드하면 렌더링 성능을 높일 수 있을 것입니다.

뷰포트 밖에 있는 이미지는 사용자가 스크롤할 때 그래서 뷰포트 안에 들어올 때 그때 로드를 해도 돼서 다운로드를 늦게 해도 될 것입니다. 코드에 가서 이미지를 좀 지연 로드하는 설정을 추가해보겠습니다.
...
이미지 태그에 `loading`이라는 속성이 있습니다. 이걸 사용하면 지연 로드를 사용할 수 있습니다.

기본값은 `loading`이 `eager`여서, 항상 모든 자원들을 다운로드하게 되어있습니다.
...
이걸 모두 `lazy`로 변경해서, 그러면 이 이미지 태그가 뷰포트 안에 들어올 때만 HTTP 요청을 만들게 됩니다. 이렇게 하고 다시 확인해보겠습니다.
