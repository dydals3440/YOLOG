...
![렌더링 최적화](/images/http-rendering-optimization/thumbnail.png)
...
먼저 **파싱 단계**입니다. HTML 문서를 파싱해서 `DOM`을 만들고, CSS 코드는 `CSSOM`으로 만듭니다.

다음은 **렌더 트리 생성**입니다. DOM과 CSSOM을 합쳐서 렌더 트리를 만듭니다.
...
`async`는 스크립트를 동시에 다운로드하고 완료된 순서대로 실행합니다. script-big.js를 먼저 작성했지만, script-small.js가 빨리 다운로드되어 먼저 실행됐습니다.

`async`는 DOM과 무관하고 서로 독립적인 스크립트에 적합합니다. 광고 스크립트나 분석 도구처럼 애플리케이션과 무관한 스크립트에 사용하면 좋습니다.
