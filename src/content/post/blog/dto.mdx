...
`renderSummary`는 `title`이나 `scheduledTime`에 관심이 없는데, `todos` 전체를 받고 있죠. 나중에 할 일 목록 쪽 요구로 `title` 타입이 바뀌거나 `scheduledTime` 형식이 달라지면? `renderSummary`는 건드린 적도 없는데 갑자기 검토 대상이 됩니다.
...
interface Todo {
  ...
  scheduledTime: string | null;
  startDate: string;
  items: { id: number; title: string; completed: boolean }[];  // 추가됨 // [!code ++]
  itemStats: { total: number; completed: number };              // 추가됨 // [!code ++]
}
```
...
| 서버 DTO (`TodoResponse`) | 클라이언트 모델 (`TodoItem`) | 변환 |
|-------------------|---------------------------|------|
...
export const toTodoItem = (dto: TodoResponse): TodoItem => ({
  id: dto.id,
  title: dto.title,
  // ... 단순 매핑 필드 생략
