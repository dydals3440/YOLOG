...
요즘은 대부분 회사가 React Query를 도입하기도 하고, 대부분의 서버 데이터를 불러오는 강의들은 모두 React Query를 기반으로 진행하기 때문에 나처럼 늦게 개발을 시작한 사람에게는 React Query 말고 다른 것을 경험해보기가 쉽지 않다.

React Query가 지금처럼 주류의 기술로 떠오르기 이전에는 서비스의 특성과 개발자의 취향에 따라, [redux-thunk](https://github.com/reduxjs/redux-thunk), [redux-saga](https://redux-saga.js.org/) 등 다양한 비동기 미들웨어를 채택하며 사용했고 더 효율적인 업무를 위하여 미들웨어 자체를 직접 커스텀하며 사용했다고 한다.
...
React는 이러한 부수 효과를 처리하기 위해 내장 훅인 `useEffect`를 제공하며, 이를 통해 네트워크 통신과 같은 비동기 작업을 안전하게 수행하도록 한다.
...
1. **기술의 원리 분석**: 현재 사용하고 있는 라이브러리와 프레임워크들을 분해하고 분석하는 과정을 통해, 그들이 해결하고자 했던 근본적인 문제와 설계 의도를 깊이 이해하는 데 집중하고자 한다.
2. **생태계 기여**: 오픈 소스 프로젝트에 기여하며 경험을 쌓고, 궁극적으로 개발 커뮤니티에 유용하고 혁신적인 번들러나 라이브러리를 직접 만들어 제공할 수 있는 실력을 갖추고자 한다.

많이 힘들겠지만, 서비스 개발의 보람과 개발 환경 개선의 가치를 모두 추구하며, 기술 생태계에 긍정적인 영향을 미치는 개발자로 성장해 보고 싶다.

:::note
**핵심 정리**

- **useEffect의 한계**: 서버 데이터 패칭 시 로딩, 에러, 캐싱, 재시도 등 반복적인 로직 필요
- **직접 구현의 복잡성**:
  - 캐싱: 타임스탬프 관리, staleTime/gcTime 설정
  - 요청 취소: AbortController로 불필요한 네트워크 요청 방지
  - 재시도: 지수 백오프 전략, 최대 재시도 횟수 관리
  - 경쟁 상태: 여러 요청의 순서 보장
- **React Query의 장점**:
  - 선언적 API로 복잡한 절차적 로직 추상화
  - queryKey로 자동 캐싱 및 중복 요청 제거
  - signal로 자동 요청 취소
  - retry, retryDelay로 자동 재시도
  - staleTime, gcTime으로 캐시 관리
- **핵심 개념**:
  - `queryKey`: 데이터 식별 및 캐싱 키
  - `queryFn`: 실제 데이터 패칭 함수
  - `staleTime`: 데이터 신선도 유지 시간
  - `gcTime`: 캐시 가비지 컬렉션 시간

**React Query는 마치 전문 비서와 같다. 데이터를 언제 가져올지, 얼마나 보관할지, 실패하면 몇 번 재시도할지 모두 알아서 관리해준다. 개발자는 비즈니스 로직에만 집중하면 된다.**
:::
