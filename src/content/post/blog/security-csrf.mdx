...
웹 개발자라면 꼭 알아야 할 심각한 보안 취약점 `CSRF(Cross-Site Request Forgery)`에 대해 알아보겠다.

이번 포스트에서는 직접 공격 서버를 구축하고 실제 공격을 시연한 후 `SameSite 쿠키`와 `CSRF 토큰`으로 방어하는 방법까지 단계별로 실습해보겠다.
...
이제 직접 간단하게 공격을 구현해보면서 CSRF의 위험성을 체험해보겠다.
...
이전 포스트의 [세션 하이재킹](https://www.yolog.co.kr/post/security-session-hijacking) 실습 환경을 활용한다.
`matthew.com`이 결제 기능을 제공한다고 가정하고, 공격자가 이를 악용하는 상황을 재현해보겠다.
...
간단한 HTML을 제공하고, HTML을 보면 바디에 이미지가 하나 있다. 그런데 이 이미지는 바로 공격 대상인 `matthew.com`으로 네트워크 요청을 보내는 이미지 태그입니다.
...
공격용 웹 페이지를 제공하는 공격 서버에서 이 URL로 요청을 보내면, 다른 사용자의 결제를 대신 해버릴 수도 있다. 만약 `matthew.com`이 결제할 수 있는 URL을 제공한다고 하면, 문제가 더 심각해질 수 있다.

### 브라우저 보안 설정 해제 (실습용)

실습을 위해 일시적으로 브라우저 보안을 해제한다. `Firefox`의 향상된 추적 방지 부분에서 사용자 지정으로 해서 쿠키 체크박스를 해제할 것입니다.
...
> 최신 브라우저들은 기본적으로 `CSRF 공격`을 어느 정도 방어한다.
> 실습을 위해 이러한 보호 기능을 일시적으로 해제한다.
...
4. 만약 이 요청이 결제 프로세스라면 `matthew.com`에서는 인증된 요청이라고 생각하고 실행할 수도 있다는 것을 보았다!
...
간단한 해결방법은 쿠키 디렉티브를 설정하는 것입니다. 이전 [쿠키 편](https://www.yolog.co.kr/post/http-cookie)에서 본 것처럼 `SameSite` 디렉티브를 사용하면 쿠키를 사용할 수 있는 사이트를 제한할 수 있다.

`SameSite` 속성은 쿠키가 전송되는 조건을 제한한다:
...
이렇게 `SameSite=Strict` 속성이 잘 들어감을 확인할 수 있다.

다시 `hacker.com:8082`로 들어가서 요청을 보내보겠다:

![SameSite 적용 후: 쿠키가 전송되지 않음](/images/security-csrf/6.webp)

이미지 태그를 발견하고 `matthew.com` 쪽으로 요청을 보내는데, 이때 쿠키 값을 보면 SID 쿠키가 없다.
...
더이상 브라우저는 `matthew.com`이 아닌 곳에서는 이 쿠키를 보내지 않다.
...
이 `SameSite` 쿠키는 MDN 문서에서 한 번 자세하게 읽어보는 것이 좋다.

![MDN: SameSite 쿠키 문서](/images/security-csrf/7.webp)

이런 식으로, 아직 실험 중인 기법입니다.
아직은 여전히 실험 중이면서 모든 브라우저에 제공되지 않고 있다고 한다.
...
그래서 다른 예방책이 있다. 특별한 토큰을 `HTML` 문서에 주입하고, 다음 요청 시에 이 토큰이 HTTP 요청에 들어오는지 서버가 확인하는 방식입니다.
...
이렇게 `matthew.com`에서 제공한 HTML에만 있는 토큰을 네트워크 헤더에 실어서 보내면, 서버는 정상적인 요청이라고 판단할 수 있다.
...
네, 저도 압니다. 너무 깁니다. 그냥 "사람과 컴퓨터를 구별하는 자동화된 테스트"라고 생각하시면 된다.
...
구글의 reCAPTCHA 같은 서비스를 통해 자동화된 공격을 차단할 수 있다:

![Google reCAPTCHA 예시 - CAPTCHA는 이런 것을 의미해요](/images/security-csrf/9.webp)

사용하는 방법은 아래와 같다. 매우 간단한 예제로 직접 만들어보겠다:
...
솔직히 CSRF 공격이라는 게 처음엔 "이게 진짜 가능해?"싶었는데 직접 실습해보니 정말 간단한 `img` 태그 하나로도 공격이 가능한 것을 확인할 수 있었다.

무서운 건 사용자는 전혀 눈치채지 못한다는 것이다.

나도 처음엔 보안을 너무 빡빡하게 하려다가 사용자들이 "왜 이렇게 불편해요?"라는 피드백을 많이 받았다. 캡차 넣고 재인증 넣고... 결국 보안과 사용성 사이에서 균형을 찾는 게 제일 어려운 것 같다.

요즘은 AI가 캡차도 뚫는다고 하니 완벽한 보안은 정말 없는 것 같다.

결국 내가 생각하는 좋은 서비스는 사용자가 보안 기능이 있는지도 모르면서 안전하게 쓸 수 있는 서비스인 것 같다. 마치 자동차의 에어백처럼 평소엔 신경 안 쓰다가 필요할 때 알아서 작동하는 그런 보안이다.
...
- **방어 방법 2: CSRF 토큰**:
  - 서버가 랜덤 토큰 생성 → 세션에 저장
  - HTML 폼에 hidden input으로 토큰 삽입
  - 요청 시 토큰 검증
  - 공격자는 토큰 값을 알 수 없음
- **방어 방법 3: Referer/Origin 검증**:
  - 요청의 출처 확인
  - 다른 도메인에서 온 요청 차단
  - 프록시 환경에서 Referer가 없을 수 있음
- **방어 방법 4: reCAPTCHA**:
  - 사람인지 봇인지 구별
  - 중요한 작업(송금, 비밀번호 변경)에만 적용
  - 사용자 경험 고려 필요
- **보안과 사용성의 균형**:
  - 너무 빡빡한 보안은 사용자 이탈
  - 너무 느슨한 보안은 공격 위험
...
**CSRF 공격은 피싱 사기와 같다. 은행 직원을 사칭해서(정상 사이트 요청으로 위장) 피해자 몰래 계좌이체를 실행한다. SameSite 쿠키는 신분증 확인(같은 사이트인지 검증), CSRF 토큰은 일회용 비밀번호(공격자가 알 수 없는 값)와 같다.**
