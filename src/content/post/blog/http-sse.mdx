...
SSE는 HTML5 명세에 정의된 기술로, **서버가 클라이언트로 단방향 실시간 데이터를 전송**하는 프로토콜입니다.
WebSocket과 달리 여전히 HTTP로 동작하며, 서버에서 클라이언트로만 메시지를 보낼 수 있습니다.

### 핵심 개념

SSE는 서버가 특별한 HTTP 응답 헤더 값을 응답에 싣는 것으로 시작합니다.
...
SSE는 다양한 필드를 제공합니다 ([MDN 문서](https://developer.mozilla.org/ko-KR/docs/Web/API/Server-sent_events/Using_server-sent_events)):
...
폴링이나 롱폴링과 비교했을 때, SSE는 더 간단하게 구현할 수 있습니다. 한번 연결하면 계속 메시지를 받을 수 있죠.
...
## 통신 기법 비교: Polling vs Long Polling vs SSE

| 구분              | [Polling](https://www.yolog.co.kr/post/http-polling)                   | [Long Polling](https://www.yolog.co.kr/post/http-long-polling)     | SSE                      |
| ----------------- | ------------------------- | ---------------- | ------------------------ |
...
SSE는 **서버에서 클라이언트로 실시간 데이터를 푸시**하는 가장 간단한 방법입니다.
...
양방향 통신이 필요 없다면, SSE는 여전히 훌륭한 선택지입니다.

다음 포스트에서는 진정한 양방향 실시간 통신을 가능하게 하는 **WebSocket**을 다루겠습니다.
