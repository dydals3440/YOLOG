...
W3C에서는 이를 예방할 수 있는 기술로 [`Content-Security-Policy`](https://developer.mozilla.org/ko/docs/Web/HTTP/Headers/Content-Security-Policy)라는 응답 헤더를 표준으로 정의했다.
...
브라우저 콘솔에는 **Content Security Policy** 경고가 표시된다. "다음 지시자를 위반했기 때문에 인라인 스크립트가 실행되지 않도록 차단했습니다"라는 메시지를 볼 수 있다. 이는 브라우저가 CSP 정책을 올바르게 이행한 것이다.
...
- **[`media-src`](https://developer.mozilla.org/ko/docs/Web/HTTP/Headers/Content-Security-Policy/media-src)**: 오디오/비디오는 `example.org`와 `example.net`에서만 허용
...
[`Content-Security-Policy-Report-Only`](https://developer.mozilla.org/ko/docs/Web/HTTP/Headers/Content-Security-Policy-Report-Only) 응답 헤더를 사용한다:
