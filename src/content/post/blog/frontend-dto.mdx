...
잠재적으로 데이터베이스에 저장되어 있는 날 것, 민감한 데이터를 안전하게 소비할 수 있는 구조로 변환하는 수단 역할을 하기 때문이다.

이번 글에서는 프론트엔드에서도 DTO가 필요한 이유를 설명한다. 그리고 백엔드의 DTO와 프론트엔드의 DTO는 목적이 다르다는 점도 함께 다룬다.

단, 프론트엔드에서 DTO를 쓰는 이유는 서버와 비슷한 보안을 위한 것이 아니다. 어차피 프론트엔드에서 다루는 데이터는 대부분 Public하다.

핵심은 **서버 응답을 프론트엔드가 쓰기 편한 형태로 변환**하는 것이다.

---

## 잘만든 백엔드 DTO에 프론트엔드 DTO 얹기
...
물론, 지금 위의 예시처럼 컴포넌트 안에서 상태를 파생(derive)하는 게 무조건 잘못된 건 아니다.
...
관련된 로직은 **한 곳에 응집**되어 있을 수록 **같이 일하는 동료들이 코드를 이해하고 수정하기 쉽다.**
...
타입과 필드 이름은 **사용하는 개발자 관점**으로 짓는 게 유지보수에 좋다.
...
[TanStack Query](https://github.com/TanStack/query) 래퍼를 만들 때, 피처 기반 디렉토리 구조로 관리한다:
...
└── features/
    └── user/
        ├── models/
        │   └── user.model.ts           # API 응답 스키마 (Zod)
        ├── repositories/
        │   └── user.dto.ts             # DTO 변환 로직
        └── presentations/
            └── hooks/
...
import { UserResponseSchema } from '@/features/user/models/user.model';
import { userDto, type UserDTO } from '@/features/user/repositories/user.dto';
...
export function useUserQuery(userId: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<UserDTO> => {
      ...
      // 1. 런타임 검증
      const decoded = UserResponseSchema.parse(json);

      // 2. DTO 변환
      return userDto(decoded);
    },
  });
}
```
...
이제 처음에 봤던 Bad 예제가 어떻게 바뀌는지 보자:

```tsx
// 파생 로직이 완전히 사라진다!
const GrandParent = () => {
  const { data: user } = useUserQuery('123'); // 이미 DTO로 변환됨
