...
> **비유로 이해하기**  
> 음식 배달 앱에서 주문 상태를 확인하는 상황을 생각해보세요.
>
...
5. 만약 일정 시간(예: 10초) 동안 새 데이터가 없으면 서버가 `408 Request Timeout`을 보내고 연결을 종료합니다.  
   클라이언트는 다시 요청을 시도합니다.
...
3. **브로드캐스트**: 새 메시지가 오면 대기 중인 모든 클라이언트에게 동시에 전달합니다. 실시간 채팅의 핵심입니다.
...
3. **타임아웃 처리**: 408 응답을 받으면 에러가 아니라 정상 흐름으로 처리합니다.
...
- **서버 리소스 부담**: 연결을 오래 유지 = 메모리/스레드 점유
- **확장성 문제**: 동시 접속자가 많으면 서버 부하 급증
- **연결 관리 복잡**: 타임아웃, 재연결, 에러 처리 등 고려사항 많음
- **단방향 통신**: 서버→클라이언트 푸시만 가능 (양방향 실시간 통신 제한적)
...
- **롱 폴링이 여전히 사용되는 경우**:
  ...
  - 프록시/방화벽이 엄격한 기업 환경
  - 간헐적인 업데이트만 필요한 서비스
  - 실제로 Facebook, Gmail 등도 부분적으로 롱 폴링 사용한다고 합니다.
