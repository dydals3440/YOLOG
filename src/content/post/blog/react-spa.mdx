...
- **History API의 등장**: HTML5에서 등장하여 전체 리로드 없이 URL만 변경 가능
- **pushState**: 히스토리 스택에 새로운 상태를 추가, 네트워크 요청 발생 안 함
- **popstate 이벤트**: 뒤로가기/앞으로가기 시 발생, 스택 포인터만 이동
- **SPA 라우팅 원리**:
  - Link 컴포넌트: preventDefault()로 새로고침 방지, pushState()로 URL 변경, 커스텀 이벤트로 알림
  - Router 컴포넌트: pushstate/popstate 이벤트 감지, 현재 URL에 맞는 컴포넌트 렌더링
- **핵심 개념**:
  - `history.pushState()`: URL 변경 (네트워크 요청 X)
  - `popstate 이벤트`: 브라우저 뒤/앞으로 가기 감지
  - `preventDefault()`: 기본 동작 방지 (페이지 새로고침)
  - 커스텀 이벤트: 앱 전역에 경로 변경 알림
- **React Router vs 직접 구현**:
  - React Router: 중첩 라우팅, 동적 세그먼트, 데이터 로딩 등 고급 기능
  - 직접 구현: 핵심 원리 이해, 문제 상황 대응력 향상

**SPA 라우팅은 마치 책의 목차와 같다. 페이지를 넘기지 않고(새로고침 없이) 목차만 보고(URL만 변경) 원하는 내용으로 바로 이동할 수 있다.**
