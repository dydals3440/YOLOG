...
그런데 정적 SPA를 CDN에 얹는 일은 파일만 올린다고 끝이 아니다. 배포가 한 번 나갈 때마다 세 가지가 어긋날 수 있다.

- 새 배포가 나가면 옛 `index.html`을 캐시한 브라우저는 이미 사라진 옛 청크를 가리킨다. 어떻게 막을까?
- `/academy/1/cards/list` 같은 라우트는 `S3`에 그런 파일이 없다. 사용자가 그 주소에서 새로고침하면 어떻게 될까?
- 배포 도중 옛 탭을 켜둔 사용자가 받으려던 청크가 그새 사라지면?

이 세 질문에 답하는 게 사실상 정적 SPA 배포의 절반이다. 우리는 이를 세 축으로 풀었다 — ① 파일마다 다른 캐시 수명, ② 옛 에셋을 함부로 지우지 않기, ③ 없는 경로를 위한 `SPA 폴백`. 지금부터 차례로 본다. 먼저 캐시 수명이다.
...
두 헤더는 한 쌍으로 움직인다. `index.html`은 매번 새로 받아 늘 최신 해시 파일을 가리키고, 그 해시 파일은 이름 자체가 곧 버전이라 1년을 캐시해도 충돌이 없다. 그래서 "항상 최신인 진입점 + 영원히 고정인 에셋"이라는 조합이 성립한다.
...
여기서 무효화 경로를 `/*`가 아니라 `/`·`/index.html`로 좁힌 것도 의도다. 해시 붙은 에셋은 내용이 바뀌면 파일명까지 바뀌니 애초에 무효화할 필요가 없다. 오히려 `/*`로 엣지를 통째로 비우면, 배포 직후 이미 열려 있는 옛 탭이 기댈 옛 에셋까지 엣지에서 사라진다. 실제로 바뀐 건 `index.html` 하나뿐이니, 딱 그 하나만 비우면 된다.

그런데 `CloudFront`에서 꼭 해둬야 하는 게 두 가지 있다.

- `SPA 폴백` — `/academy/1/cards/list` 같은 라우트는 `S3`에 파일이 없다. `403/404 → /index.html (200)`로 매핑해 SPA가 라우팅을 잇게 한다. (`OAC`로 막힌 버킷은 [`404`](https://developer.mozilla.org/ko/docs/Web/HTTP/Status/404)가 아니라 [`403`](https://developer.mozilla.org/ko/docs/Web/HTTP/Status/403)이 떨어지므로 둘 다 잡아야 한다.)
