...
하나는 **비용**이다. Vercel 요금은 빌드나 배포 횟수가 아니라 트래픽과 팀 좌석에 붙는데, 둘 다 쌓이면서 무시하기 어려운 고정비가 되어갔다.
...
> 2026년 6월 3일 기준으로 [Claude](https://claude.ai)의 도움을 받아 대략적인 비용을 비교해봤다. 단가는 언제든 바뀔 수 있으니 흐름만 참고하자.
...
차이의 핵심은 두 가지다. Vercel은 트래픽이 적어도 **좌석당 월 $20**이라 3명이면 $60이 트래픽과 무관하게 깔린다. 반면 `CloudFront`는 **1TB 전송과 1천만 요청이 영구 무료**라, 내부 도구 수준의 트래픽은 사실상 무료 구간에 들어간다. 트래픽이 커지면 둘 다 전송비가 지배하지만, GB당 단가도 `CloudFront`(약 $0.12)가 Vercel($0.15)보다 낮다.
...
on:
  ...
  workflow_dispatch:        # 수동 재배포용
    inputs:
      ...
      teacher_target:
        description: "teacher-web 배포 환경 (staging | prod)"
```
...
우리는 이 목록을 미리 고정하지 않고 `detect` 단계에서 "영향받은 앱"만 골라 동적으로 만든다. 그래서 `admin`만 바뀐 푸시는 `admin` 잡 하나만, 둘 다 바뀌면 두 잡이 병렬로 도는 식이다.
  ...
  [ .[] |
    if . == "admin-web" then
      ...
      if $tt == "prod" then
        ...
        { app: ., environment: "staging-teacher", label: "staging" }   # teacher → 기본 staging
      end
    else empty end
  ]')
```

읽어보면 의도가 그대로 보인다. `admin-web`은 `production-admin`으로 **직행**하고, `teacher-web`은 기본적으로 `staging-teacher`로 간다. 왜 이렇게 다른지는 뒤에서 자세히 다룬다.
...
on:
  workflow_call:
    inputs:
      ...
      environment:  { type: string, required: true }   # production-admin | staging-teacher | ...
...
- name: Build
  env:
    ...
    VITE_SENTRY_DSN: ${{ vars.VITE_SENTRY_DSN }}             # 에러 추적
    VITE_AMPLITUDE_API_KEY: ${{ vars.VITE_AMPLITUDE_API_KEY }} # 분석
    # ... 그 외 환경별로 갈아 끼우는 변수들
  run: pnpm exec turbo run build --filter=@crabit/${{ inputs.app }}
```

`staging-teacher`와 `production-teacher`가 서로 다른 API를 바라보는 건 이 한 줄 덕분이다. 빌드 본문은 같고, [GitHub Environment](https://docs.github.com/ko/actions/deployment/targeting-different-environments/using-environments-for-deployment)의 변수만 갈아 끼운다.
...
그 역할은 우리 저장소의 `main`에서 온 요청에만, 해당 버킷·distribution에 대한 최소 권한만 갖도록 잠가뒀다.
...
핵심은 **Scope를 `assets/` prefix로 좁힌 것**이다. 해시 붙은 청크는 전부 `assets/` 아래에 있고, `index.html`·`favicon` 같은 진입점 파일은 그 prefix 밖이라 만료 대상에서 안전하게 빠진다. 만약 진입점까지 만료되면 사이트 자체가 사라지니, 이 경계가 중요하다.
...
- **SPA fallback** — `/academy/1/cards/list` 같은 라우트는 `S3`에 파일이 없다. `403/404 → /index.html (200)`로 매핑해 SPA가 라우팅을 잇게 한다. (`OAC`로 막힌 버킷은 404가 아니라 403이 떨어지므로 둘 다 잡아야 한다.)
...
앞서 본 매트릭스 분기, 그리고 본체의 `environment:` 한 줄이 만나면 두 앱의 배포 속도가 갈린다. `admin`은 머지하면 곧장 프로덕션, `teacher`는 staging을 거쳐 승인해야 프로덕션이다. 같은 파이프라인을 쓰는데 왜 다르게 굴릴까? 답은 두 앱이 짊어진 리스크가 다르기 때문이다.
...
flowchart TD
    ...
    TE --> TS[staging-teacher<br/>Route 53 전용 URL]
    TS --> IT[내부 테스트]
    IT --> EX{외부 심사}
    EX -.->|심사 중엔 staging 유지| TS
    EX -->|통과 후 승인| GATE[내부 검토<br/>승인 게이트]
    GATE --> TP[production-teacher]
```

### admin은 빠른 주기가 중요해 staging을 두지 않았다

`admin-web`은 내부 운영자용 도구다. 사용자가 한정돼 있고, 문제가 생겨도 영향 범위가 팀 안이다. 무엇보다 고친 걸 **빨리 반영하는 짧은 배포 주기**가 곧 가치다. 그래서 굳이 staging을 두지 않았다. `main` 머지 = `production-admin` 직행, 중간 단계가 없다.

### teacher는 외부 서비스 연동 때문에 staging이 필수다

`teacher-web`은 실제 선생님들이 쓰는 서비스다. 잘못 나가면 외부 사용자가 곧바로 영향을 받는다. 게다가 토스페이먼츠(결제), 팝빌(예금주명 조회), 싸인오케이(전자서명), 카카오 같은 외부 서비스를 여럿 물고 있다.

이 중 일부는 **연동 전에 심사를 거쳐야 한다.** 서비스를 미리 만들어 두고, 실제로 잘 동작하는지 보여줘야 통과된다. 그래서 `staging-teacher`에 `Route 53`으로 전용 URL을 붙여 두고, 그 환경에서 내부 테스트와 심사를 진행한다. 심사를 통과하고 최종 승인이 나면, 한 번 더 내부 검토를 거쳐 비로소 배포 서버로 올린다.

그래서 `main` 머지는 일단 `staging-teacher`까지만 자동으로 가고, 그 뒤는 승인을 받아야 프로덕션으로 나간다.

프로덕션 승격은 별도 잡이 담당한다.

```yaml
promote-teacher-prod:
  needs: [detect, deploy]
  if: |
    github.event_name == 'push' &&
    needs.detect.outputs.has_teacher == 'true' &&
    needs.deploy.result == 'success'
  uses: ./.github/workflows/deploy-aws.yml
  with:
    app: teacher-web
    environment: production-teacher      # ← 이 환경에 "승인 게이트"가 걸려 있다
    target_label: prod
  secrets: inherit
```

여기서 승인 게이트의 정체가 드러난다. 본체의 `environment: ${{ inputs.environment }}`가 `production-teacher`를 가리키는 순간, `GitHub Environments`에 설정한 `Required reviewers`가 작동한다. 승인자가 Approve를 누르기 전까지 이 잡은 멈춰서 기다린다. 덕분에 `teacher`의 프로덕션 배포는 staging에서 검수하고 승인했을 때만, 의도적이고 묶음 단위로 일어난다.
...
특히 `teacher`가 심사 중일 때 빛을 본다. 그 사이 `admin` 기능을 고쳐 배포해도, 영향받은 앱만 골라 내보내고 `teacher`엔 승인 게이트가 걸려 있어 **심사 중이던 `teacher`가 함께 딸려 나가는 일이 없다.** 반대로 `teacher`가 검수 대기로 멈춰 있어도 `admin`은 평소처럼 배포된다.
...
| 잃은 것 | 보완책 |
|---|---|
| PR 단위 자동 Preview | `teacher`는 `staging-teacher`, `admin`은 prod 자체가 빠른 피드백 루프 |
...
- **무엇이 달랐나** — 두 앱에 각자 어울리는 속도를 입혔다. `admin`은 빠르게 직행하고, `teacher`는 외부 서비스 검증을 위해 staging을 거친다.
